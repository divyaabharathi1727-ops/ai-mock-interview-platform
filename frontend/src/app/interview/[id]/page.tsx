...
function SessionContent() {
  ...
  async function handleAnswer() {
    ...
    try {
      const response = await submitAnswer(interviewId, question.id, { answerText: answer.trim() });
      setSession({
  ...
  return <main className="page-shell"><nav className="topbar"><Link className="brand" href="/dashboard"><span className="brand-mark">●</span> Interview Lab</Link><Link className="text-link" href="/history">Exit session</Link></nav>{isLoading ? <div className="page-state">Starting interview...</div> : error && !session ? <section className="dashboard"><p className="form-error" role="alert">{error}</p><Link className="text-link" href="/history">Return to history</Link></section> : <section className="session-shell"><div className="session-heading"><div><span className="eyebrow">Mock interview</span><h1>{interview?.jobRole}</h1><p>{interview?.interviewType} interview · {interview?.difficulty}</p></div><span className="eyebrow">Development question bank</span></div>{error && <p className="form-error" role="alert">{error}</p>}<div className="session-progress"><div className="progress-label"><span>Question {session && question ? question.questionOrder : session?.totalQuestions} / {session?.totalQuestions}</span><span>{session?.answeredQuestions} answered</span></div><div className="progress-track"><div className="progress-value" style={{ width: `${progress}%` }} /></div></div>{question ? <div className="question-card"><span className="question-type">{question.questionType}</span><h2>{question.questionText}</h2><label htmlFor="answer">Your answer</label><textarea id="answer" value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Take a moment, then write your answer..." maxLength={10000} disabled={isSubmitting} /><div className="answer-footer"><span>{answer.length} / 10000</span><button className="primary-button" onClick={handleAnswer} disabled={isSubmitting}>{isSubmitting ? "Submitting answer..." : "Submit answer"}</button></div></div> : <div className="question-card"><span className="question-type">Ready</span><h2>You have answered every question.</h2><p className="muted-copy">Review your responses, then finish this session when you are ready.</p></div>}{session?.readyToFinish && <button className="finish-button" onClick={handleFinish} disabled={isFinishing}>{isFinishing ? "Finishing interview..." : "Finish interview"}</button>}</section>}</main>;
}
